...
final class DockClickWatcher {
    ...
    private func handle(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        case .leftMouseDown:
            ...
            // can re-activate the app — then hide it so it just disappears and
            // whatever is behind comes forward (Supercharge-style, Cmd-H).
            swallowNextMouseUp = true
            let name = front.localizedName ?? "app"
            status = front.hide() ? "Hid \(name) ✓" : "Couldn’t hide \(name)"
            return nil
